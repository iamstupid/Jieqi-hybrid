...
// Combines 128-bit values into concatenated hash.
inline uint64_t HashCat(std::initializer_list<__uint128_t> args) {
  uint64_t hash = 0;
  for (__uint128_t x : args) {
    hash = HashCat(hash, uint64_t(x >> 64));
    hash = HashCat(hash, uint64_t(x));
  }
  return hash;
}
